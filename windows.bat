@echo off
set FILEPATH=%USERPROFILE%\Documents\exemple_element.txt
set FILENAME=client_push.txt
set SERVER=http://localhost:3100/deploy-heavy-element

echo Envoi du fichier vers le backend...

powershell -Command ^
  "$content = Get-Content '%FILEPATH%' -Raw; ^
   $encoded = [Convert]::ToBase64String([System.Text.Encoding]::UTF8.GetBytes($content)); ^
   $json = @{fileName='%FILENAME%'; base64Content=$encoded} | ConvertTo-Json; ^
   Invoke-RestMethod -Uri '%SERVER%' -Method POST -Body $json -ContentType 'application/json'"

pause
// BACKEND Node.js avec Express, WebSocket et enregistrement intranet + module PHANTOMS décentralisé + framework d'entrée 2 avec relais intelligent + apprentissage autonome + interface de navigation inter-systèmes avec modules output
...
app.get('/', (req, res) => {
  ...
  <head>
    ...
    <title>Interface de Navigation Inter-Systèmes</title>
    <style>
      ...
      button, select { margin-top: 10px; padding: 10px; }
    </style>
  </head>
  <body>
    <h1>Interface Navigatrice</h1>
    <label for="systemSelect">Système :</label>
    <select id="systemSelect">
      <option value="windows">Windows</option>
      <option value="linux">Linux</option>
      <option value="macos">macOS</option>
    </select>
    <button onclick="triggerOutput()">Activer Module Output</button>
    <div id="log"></div>
    <script>
      function triggerOutput() {
        const system = document.getElementById('systemSelect').value;
        const now = new Date().toLocaleString();
        document.getElementById('log').innerHTML += `<p>[${now}] Module output activé pour ${system}</p>`;
      }
    </script>
  </body>
  </html>
